...
import { PlusCircle, TicketPercent, ShieldCheck, Sparkles, RefreshCw } from 'lucide-react';
      ...
      <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-sm text-zinc-400 mt-1 max-w-xl">
            Real-time fulfillment metrics, inventory velocity, automated coupon campaigns, and live database insights.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          {onRefresh && (
            <button
              ...
              onClick={onRefresh}
              disabled={isLoading}
              className="inline-flex items-center gap-2 px-3.5 py-2.5 rounded-xl text-xs font-semibold bg-zinc-800 hover:bg-zinc-700 text-zinc-200 border border-zinc-700 hover:border-zinc-600 transition-all cursor-pointer disabled:opacity-50"
              title="Refresh live metrics from database"
            >
              <RefreshCw className={`w-3.5 h-3.5 text-zinc-300 ${isLoading ? 'animate-spin' : ''}`} />
