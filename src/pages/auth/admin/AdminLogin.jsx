  ...
  const handleLogin = async (values, { setSubmitting }) => {
    try {
      ...
      if (response?.data?.success && response?.data?.data?.user) {
        ...
        // Update Redux authentication state (tokens are securely kept in HttpOnly cookies)
        dispatch(setAuthUser(loggedUser));

        toast.success(
          `Access Authorized. Welcome back, ${
            loggedUser.firstName || "Administrator"
          }!`
        );
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <button
                      ...
                      className="absolute inset-y-0 rounded right-0 pr-3 flex items-center text-zinc-500 hover:text-zinc-300 transition-colors cursor-pointer"
                      tabIndex={-1}
                      aria-label={
                ...
                <div className="flex items-center justify-between pt-1">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      className="w-3.5 h-3.5 rounded bg-zinc-950 border-zinc-800 text-indigo-600 focus:ring-indigo-500 focus:ring-offset-zinc-900 cursor-pointer"
                    />
