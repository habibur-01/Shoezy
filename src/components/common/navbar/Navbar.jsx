...
const Navbar = () => {
  ...
  console.log("🚀 ~ Navbar ~ cartItemNum:", cartItemNum)
  ...
  const loadCartItem = async () => {
    try {
      if (hasUser) {
        const result = await totalCartItem();
        if (result?.data?.success) {
          dispatch(setCountCartItem(result.data?.data))
        }
      }
    } catch (error) {
      dispatch(setCountCartItem(0))
    }
  ...
  return (
    <div className="w-full h-28 bg-[var(--color-background)] py-3 px-20 flex justify-between items-center">
...

          {
            ...
            categories.map((navItem) => (
              <li key={navItem?._id} className="relative group">
                <NavLink
                  to={navItem?.subcategories?.length > 0 ? "#" : `/products/${navItem.slug}`}
      ...
      <div className="flex items-center gap-x-8">
        ...
        <NavLink className="flex items-center gap-x-2 hover:cursor-pointer" to={'/account/mycart'}>
          <div className="relative">
            <MdOutlineShoppingCart className="text-3xl" />
            <div className="bg-[var(--color-black)] w-5 h-5 flex justify-center items-center aspect-square rounded-full absolute top-0 left-1/2 -translate-y-1/3 translate-x-1/12">
              <p className="text-white text-xs">{cartItemNum || '0'}</p>
            </div>
          </div>
          <p>My Cart</p>
        </NavLink>
